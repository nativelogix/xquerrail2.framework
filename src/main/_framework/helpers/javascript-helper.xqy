...
: Converts dateTime to epoch date
 :)
declare %private function helper:dateTime-to-epoch(
  $dateTime as xs:dateTime
) {
  xs:unsignedLong(
    ($dateTime - xs:dateTime('1970-01-01T00:00:00')) div xs:dayTimeDuration('PT1S')
  )
};
...
:  Converts date to epoch date
 :)
declare %private function helper:date-to-epoch(
  $date as xs:date
) {
  xs:unsignedLong(
    (fn:dateTime($date,xs:time("00:00:00")) - xs:dateTime('1970-01-01T00:00:00')) div xs:dayTimeDuration('PT1S')
  )
};

declare function helper:date(
  $date as xs:date
) {
  helper:literal("new Date(" || helper:date-to-epoch($date) ||")")
};

declare function helper:dateTime(
  $dateTime as xs:dateTime
) {
  helper:literal("new Date(" || helper:dateTime-to-epoch($dateTime) || ")")
};

declare function helper:literal(
  $value as item()
) {
...
: Function automatically generates json string
 :)
declare function helper:json(
  $value as item()
) {
  if($value instance of json:object or $value instance of json:array) then
    xdmp:to-json($value)
  else
    xdmp:to-json(helper:entry("value",$value))
};
...
declare function helper:object(
  ...
  return (
    ...
    return
      if($value instance of json:object or $value instance of json:array) then
        xdmp:set($obj,$obj + $value)
      else
        fn:error(xs:QName("NON-JSON-TYPE"),"The type you provided is not json")
      ,
      $obj
  )
};
...
declare function helper:keyvalue(
  $key as xs:string,
...
declare function helper:variable(
  $key as xs:string,
  $json as item()*
) {
  if($json instance of json:object or $json instance of json:array) then
    fn:concat("var ", $key, " = ", xdmp:to-json($json),";")
  else
    fn:error(xs:QName("JSON-VARIABLE-TYPE"),"JSON Variable must be of type json:object or json:array")
};
